...
def get_ols_regression(x, y):
    ...
    coef, shift = np.linalg.lstsq(A, y, rcond=None)[0]

    return shift, coef


def get_mean_regression(x, y):
    mean_x = np.mean(x)
    mean_y = np.mean(y)
...
def get_symbol_pairs(symbols: List[str], shuffle=True):
    if shuffle:
    ...
    result = list(itertools.combinations(symbols[::-1], 2))
