...
class Storage:
    ...
    def __init__(self):
        ...
        if "STORAGE_HOST_KEY" in os.environ:
            ...
            ssh.get_host_keys().add('[109.87.62.99]:2222', 'ssh-ed25519', key)
    ...
    def _exists_remote(self, key):
        remote_path = os.path.join("/upload/cns_analytics/", key)
        try:
    ...
    def _exists_locally(self, key):
        local_path = os.path.join(".cache/", key)
    ...
    def _get_key(symbol: Symbol, md_type: MDType) -> str:
        return f"{symbol.name}&{symbol.exchange.name}&{md_type.name}"
