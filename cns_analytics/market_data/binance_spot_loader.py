...
async def main():
    symbols = await DataBase.get_all_symbols(exchange=Exchange.BinanceSpot)

    symbol = await DataBase.create_symbol('BNBUSDT', Exchange.BinanceSpot)

    async with BinanceSpotLoader() as bfl:
        await bfl.fetch(
            symbol=symbol,
            md_type=MDType.OHLC,
    ...
    #         symbol.exchange = Exchange.BinanceSpot
